...
void GEP::registerParameters(StateP state){
...
bool GEP::initialize(StateP state){
	...
	GEPChromosome::GEPChromosome* gen(new GEPChromosome::GEPChromosome);
...
bool GEP::advanceGeneration(StateP state, DemeP deme){
...
void GEP::invert(StateP state, const std::vector<IndividualP>& pool){
	for (uint i = 0; i < pool.size(); i++) {
		if (state_->getRandomizer()->getRandomDouble() <= invRate_) {
            ...
            GEPChromosome::GEPChromosome* individual = (boost::static_pointer_cast<GEPChromosome::GEPChromosome> (pool.at(i)->getGenotype())).get();
...
void GEP::invertDc(StateP state, const std::vector<IndividualP>& pool){
	// Do a test and check whether the Dc domain actually exists
	GEPChromosome::GEPChromosome* test = (boost::static_pointer_cast<GEPChromosome::GEPChromosome> (pool.at(0)->getGenotype())).get();
	if (test->dcLength < 1){
...
void GEP::transpose(StateP state, const std::vector<IndividualP>& pool){
...
void GEP::transposeIS(StateP state, const std::vector<IndividualP>& pool){
	for (uint i = 0; i < pool.size(); i++) {
...
void GEP::transposeRIS(StateP state, const std::vector<IndividualP>& pool){
	for (uint i = 0; i < pool.size(); i++) {
...
void GEP::transposeGene(StateP state, const std::vector<IndividualP>& pool){
	for (uint i = 0; i < pool.size(); i++) {
...
void GEP::transposeDc(StateP state, const std::vector<IndividualP>& pool){
	// Do a test and check whether the Dc domain actually exists
    GEPChromosome::GEPChromosome* test = (boost::static_pointer_cast<GEPChromosome::GEPChromosome> (pool.at(0)->getGenotype())).get();
	if (test->dcLength < 1){
